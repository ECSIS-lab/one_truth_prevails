...
def main(model_name, batch_size=2000, epochs=5):
    ...
    val_labels = np.loadtxt("./a_labels.txt")
    train_x = np.memmap("tmp.dat",dtype=traces_read.dtype,mode="w+",shape=traces_read.shape)
    train_x[:]=traces_read[:]
    del traces_read
    train_y = labels
    del labels
    ...
    with strategy.scope():
        ...
        x = conv_layer_functional(64, x)
        x = conv_layer_functional(128, x)
        x = kl.Conv1D(128, kernel_size=(3), padding='SAME')(x)
        x = kl.BatchNormalization()(x)
        x = kl.ReLU()(x)
